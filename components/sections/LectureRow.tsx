        ...
        <CollapsibleContent>
          <div className="space-y-3">
            ...
            {lecture.children.map((child, childIndex) => {
              ...
              return (
                <div key={childKey} className="mx-4 pl-4 border-l-2 border-primary/20">
                  <Collapsible className="border rounded-lg" open={isChildExpanded} onOpenChange={() => onToggleSection(childKey)}>
                    <CollapsibleTrigger asChild>
                      <div className="p-2 sm:p-3 cursor-pointer transition-colors bg-surface">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
                          {/* Title and Status Row */}
                          <div className="flex items-center gap-2 flex-shrink-0">
                            {isChildExpanded ? (
                              <ChevronDown className="h-3 w-3 sm:h-4 sm:w-4 text-primary flex-shrink-0" />
                            ) : (
                              <ChevronRight className="h-3 w-3 sm:h-4 sm:w-4 text-primary flex-shrink-0" />
                            )}
                          ...
                          {/* Meeting Details */}
                          <div className="text-xs sm:text-sm text-muted-foreground flex-1 min-w-0 ml-5 sm:ml-4">
